...
\vspace{3em}
%search desktop: beol_scaling_limits

\vspace{3em}
the moving goalposts of CMOS


\vspace{3em}
%search desktop: ultrahigh_density_logic_designs

\vspace{3em}
``[S]uccessful computers are designed for the decathlon rather than for the sprint only.'' \cite{ma2014}

\vspace{3em}
There is likely to be a market for the highest-performing cognitive system, even if that system costs more and uses more power than competitors, provided neither cost nor power are egregious.

\vspace{3em}
We have no reason to assume the highest-performing cognitive systems will reside in an earth-like environment. Performance and power consumption are both far superior in cold space. Cost will be much higher in the near term. Perhaps not in the long term.
